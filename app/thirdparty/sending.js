...
function ParcelMetaToInfo(parcelMeta) {
  const info = {};
  if (parcelMeta.destination && parcelMeta.destination.length > 0)
    info.destination = parcelMeta.destination[0];
  if (parcelMeta.origin && parcelMeta.origin.length > 0)
    info.origin = parcelMeta.origin[0];
  if (parcelMeta.type && parcelMeta.type.length > 0)
    ...
    parcelMeta.files.forEach(files => {
      if (files.file_name && files.file_name.length > 0) {
        files.file_name.forEach(file => {
          let MD5;
          if (file['MD5_HASH'] && file['MD5_HASH'].length > 0)
            MD5 = file['MD5_HASH'][0];
...
async function sendOutboxFile(fullPath, filename) {
  ...
  try {
    ...
    if (ext !== 'xml' && ext !== 'json' && ext !== 'zip' && ext !== 'ofa')
      return;
    ...
    if (content) {
      ...
      if (info.origin) {
        ...
        if (info.origin !== metaInfo.ownPhoneNumber)
      ...
      if (content.byteLength >= SIZE_LIMIT)
        attachment = data.attachment = await createAttachmentPointer(content);
      else data.content = content;

      const response = await thirdIPC('outbox_file', destination, data);
      console.log(
        'thirdPartyNode sendOutboxFile',
        fullPath,
        destination,
        response
      );
    ...
    if (attachment) {
      // Cleanup dangling attachment?
      try {
        await deleteAttachmentPointer(attachment);
      } catch (err) {
        console.warn(
          'thirdPartyNode sendOutboxFile deleteAttachmentPointer Error:',
          err,
          attachment
        );
      }
