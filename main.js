  ...
  if (!gotLock) {
    ...
    app.on('second-instance', (evt, commandLine, workingDir) => {
      ...
      if (mainWindow) {
        ...
        if (focusConv)
          mainWindow.webContents.send('open-conversation', focusConv);
      }
      return true;
    });
  }
...
async function tryInitThirdParty(ipc) {
  try {
    ...
    console.warn('tryInitThirdParty Error:', err.message || err);
  }
}
...
ipc.on('save-contact-xml', async (evt, data) => {
  ...
  const done = err => {
    ...
    if (err)
      result.error = { message: err.message, stack: err.stack, code: err.code };
