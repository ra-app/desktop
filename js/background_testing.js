...
async function inboxMessage(messageInfo) {
  ...
  if (response && response.success && response.text) {
    ...
    textsecure.messaging.queueJobForNumber(
...
const updateAdmin = async (company_id, name) => {
  const res = await apiRequest(
    '/api/v1/admin/' + company_id + '/admins/update',
    { name }
  );
...
const getClientDetails = async (company_id, client_uuid) => {
  return (await apiRequest(
    'api/v1/admin/' + company_id + '/clients/details/' + client_uuid
  )).details;
};
...
async function encryptProfile(data) {
  ...
  const dataBuffer = dcodeIO.ByteBuffer.wrap(data, 'binary').toArrayBuffer();
...
async function encryptProfileName(name) {
  ...
  // encrypt
  const encrypted = await textsecure.crypto.encryptProfileName(data, keyBuffer);
...
async function decryptProfileName(encryptedName) {
  ...
  // decrypt
  const decrypted = await textsecure.crypto.decryptProfileName(data, keyBuffer);
...
const draggableHelper = (
  ...
  const updateElementPosition = () => {
    ...
    pos[horiz] =
      horiz === 'top'
        ? top
        : window.innerHeight - top - element.clientHeight - scroll.bottom;
    pos[vert] =
      vert === 'left'
        ? left
        : window.innerWidth - left - element.clientWidth - scroll.right;
...
async function handleOfficeJSONMsg(envelope, message) {
  ...
  switch (msgData.type) {
    ...
    case 'closeTicket':
      id = msgData.msg.id;
      update = msgData.msg.update;
      setOpenCloseTicketExternalSignal(id, true, update)
      break;
    case 'openTicket':
      id = msgData.msg.id;
      update = msgData.msg.update;
      setOpenCloseTicketExternalSignal(id, false, update)
      break;
    // case 'pong':
...
// }
async function setOpenCloseTicketExternalSignal(id, state, update) {
