...
async function handleRPCOutboxFile(destination, message) {
  ...
  const result = await sendThirdPartyMsg(destination, {
    type: 'parcel',
    payload: message,
  });
...
async function handleThirdPartyEvent(event) {
  try {
    ...
    switch (msgData.type) {
      ...
      default:
        console.warn(
          'handleThirdPartyEvent - Unknown message type',
          msgData.type
        );
        break;
    }
...
async function sendThirdPartyMsg(destination, raw) {
  ...
  try {
    ...
    if (
      err.errors &&
      err.errors.length > 0 &&
      err.errors[0].name === 'OutgoingIdentityKeyError'
    ) {
